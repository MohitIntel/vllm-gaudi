...
class PROMPT_DATA:
    ...
    _data = {"image": lambda source : 
                convert_image_mode(
                    ImageAsset("cherry_blossom").pil_image \
                    if source == "default" else Image.open(source), "RGB"
                ),
            "video": lambda source : 
                VideoAsset(name="baby_reading" if source == "default" \
                else source, num_frames=16).np_ndarrays
    }
    ...
    def get_prompts(self,
        ...
        if modality == "image":
            ...
            raise ValueError(
                ...
                f" Supported modality: [image, video]"
                )
                )
...
def run_model(
    ...
    **extra_engine_args
    ...
    #    "fps": 1,
    #}
    passed_mm_processor_kwargs = extra_engine_args.get(
                                    "mm_processor_kwargs", {}
                                )
...
def start_test(model_card_path: str):
    ...
    for config in test_config:
        try:
           ...
           logger.error(f"Error during test with modality {modality}: {e}")
           raise 
